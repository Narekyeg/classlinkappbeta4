...
function handleSendResetCode() {
    ...
    if (!email) {
        showCustomAlert('Խնդրում ենք մուտքագրել էլ. փոստը', 'warning', 'Լրացման սխալ');
        return;
    }
    if (!forgotPasswordContext.role) {
        showCustomAlert('Խնդրում ենք ընտրել դերը (Աշակերտ/Ուսուցիչ) մոռացված գաղտնաբառի հղումից', 'warning', 'Սխալ հոսք');
        return;
    }
    ...
    if (!user) {
        showCustomAlert('Այս էլ. փոստով օգտատեր չի գտնվել', 'error', 'Չի գտնվել');
        return;
    }
    ...
    if (emailJsReady && !DEMO_RESET_MODE && window.emailjs) {
        try {
            ...
            window.emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
                ...
                showCustomAlert('Վերականգնման կոդը ուղարկվեց ձեր էլ. փոստին', 'success', 'Կոդ Ուղարկված');
                // Go to code step
                ...
                showCustomAlert(`Դեմո ռեժիմ: Ձեր կոդը է ${code}`, 'info', 'Դեմո Կոդ');
                document.getElementById('fp-step-email').classList.add('hidden');
                document.getElementById('fp-step-code').classList.remove('hidden');
            });
        } catch (e) {
...
function handleVerifyResetCode() {
    ...
    if (!stored) {
        showCustomAlert('Կա مشکա — կրկին ուղարկեք կոդը', 'warning', 'Ժամկետ կամ սեսիա');
        return;
    }
    const now = new Date();
    if (now > new Date(stored.expiresAt)) {
        showCustomAlert('Կոդի ժամկետը ավարտվել է: Խնդրում ենք նորից ուղարկել', 'warning', 'Ժամկետն ավարտվել է');
        return;
    }
    if (inputCode !== stored.code) {
        showCustomAlert('Սխալ կոդ', 'error', 'Սխալ Կոդ');
        return;
    }
...
function handleSetNewPassword() {
    ...
    if (!newPass || newPass.length < 6) {
        showCustomAlert('Գաղտնաբառը պետք է լինի առնվազն 6 նիշ', 'warning', 'Թույլ գաղտնաբառ');
        return;
    }
    const { role, email } = forgotPasswordContext;
    if (!role || !email) {
        showCustomAlert('Սեսիայի տվյալները կորել են. փորձեք կրկին', 'error', 'Սխալ հոսք');
        return;
    }
    let updated = false;
    if (role === 'student') {
    ...
    if (updated) {
        ...
        showCustomAlert('Գաղտնաբառը հաջողությամբ փոխվեց. Խնդրում ենք մուտք գործել նոր գաղտնաբառով', 'success', 'Պահպանված է');
        closeForgotPassword();
        if (role === 'student') {
            showStudentLogin();
        } else {
        ...
        showCustomAlert('Օգտատերը չի գտնվել. Խնդրում ենք կրկին փորձել', 'error', 'Չի գտնվել');
    }
}
